/*
================================================================================
  TEST_DB - THESIS SNOWPIPE INGESTION (AZURE) - 3 PIPE(S)
================================================================================
  Cloud:      Azure Blob Storage
  Database:   TEST_DB
  Schema:     TEST_SCHEMA
  Generated:  2026-08-30T22:50:31Z
...
--     4. Table               : TEST_DB.TEST_SCHEMA.T_CATALOG
--     5. Table               : TEST_DB.TEST_SCHEMA.T_CUSTOMERS
--     6. Table               : TEST_DB.TEST_SCHEMA.T_INVOICES
--     7. Pipe                : TEST_DB.TEST_SCHEMA.PIPE_T_CATALOG
--     8. Pipe                : TEST_DB.TEST_SCHEMA.PIPE_T_CUSTOMERS
--     9. Pipe                : TEST_DB.TEST_SCHEMA.PIPE_T_INVOICES
================================================================================
*/
...
CREATE OR REPLACE TABLE TEST_DB.TEST_SCHEMA.T_CATALOG (
...
) COMMENT = 'Data from catalog.csv'
CHANGE_TRACKING = TRUE;


-- ============================================================================
-- 8. PIPE: PIPE_T_CATALOG
-- ============================================================================

CREATE OR REPLACE PIPE TEST_DB.TEST_SCHEMA.PIPE_T_CATALOG
    AUTO_INGEST = TRUE
    INTEGRATION = 'NOTIF_INT_THESIS'
AS
COPY INTO TEST_DB.TEST_SCHEMA.T_CATALOG
FROM (
    SELECT
...
CREATE OR REPLACE TABLE TEST_DB.TEST_SCHEMA.T_CUSTOMERS (
    LOADED_AT                                          TIMESTAMP_LTZ,
    SOURCE_OF_RECORD                                   VARCHAR(16777216),
    EXTRACTED_AT                                       TIMESTAMP_LTZ,
    ROW_NUMBER                                         NUMBER(38,0),
...
) COMMENT = 'Data from customers.csv'
CHANGE_TRACKING = TRUE;


-- ============================================================================
-- 9. PIPE: PIPE_T_CUSTOMERS
-- ============================================================================

CREATE OR REPLACE PIPE TEST_DB.TEST_SCHEMA.PIPE_T_CUSTOMERS
    AUTO_INGEST = TRUE
    INTEGRATION = 'NOTIF_INT_THESIS'
AS
COPY INTO TEST_DB.TEST_SCHEMA.T_CUSTOMERS
FROM (
    SELECT
        current_timestamp() loaded_at,
        METADATA$FILENAME source_of_record,
...
CREATE OR REPLACE TABLE TEST_DB.TEST_SCHEMA.T_INVOICES (
    LOADED_AT                                          TIMESTAMP_LTZ,
    SOURCE_OF_RECORD                                   VARCHAR(16777216),
    EXTRACTED_AT                                       TIMESTAMP_LTZ,
    ROW_NUMBER                                         NUMBER(38,0),
...
) COMMENT = 'Data from invoices.csv'
CHANGE_TRACKING = TRUE;


-- ============================================================================
-- 10. PIPE: PIPE_T_INVOICES
-- ============================================================================

CREATE OR REPLACE PIPE TEST_DB.TEST_SCHEMA.PIPE_T_INVOICES
    AUTO_INGEST = TRUE
    INTEGRATION = 'NOTIF_INT_THESIS'
AS
COPY INTO TEST_DB.TEST_SCHEMA.T_INVOICES
FROM (
    SELECT
        current_timestamp() loaded_at,
        METADATA$FILENAME source_of_record,
...
================================================================================
  ...
  External Stage: EXT_STAGE_THESIS (ROOT)
      |
      +-- // --> PIPE_T_CATALOG --> T_CATALOG
      +-- // --> PIPE_T_CUSTOMERS --> T_CUSTOMERS
      +-- // --> PIPE_T_INVOICES --> T_INVOICES
...
================================================================================
  ...
  4. Test each pipe:
       ALTER PIPE TEST_DB.TEST_SCHEMA.PIPE_T_CATALOG REFRESH;
       ALTER PIPE TEST_DB.TEST_SCHEMA.PIPE_T_CUSTOMERS REFRESH;
       ALTER PIPE TEST_DB.TEST_SCHEMA.PIPE_T_INVOICES REFRESH;
