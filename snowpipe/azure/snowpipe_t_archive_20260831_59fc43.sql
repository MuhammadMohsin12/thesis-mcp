...
================================================================================
  ...
  Database:   TEST_DB
  Schema:     TEST_SCHEMA
  Generated:  2026-08-31T02:24:25Z
...
--     4. Table               : TEST_DB.TEST_SCHEMA.T_ARCHIVE
--     5. Pipe                : TEST_DB.TEST_SCHEMA.PIPE_T_ARCHIVE
================================================================================
*/
...
CREATE OR REPLACE TABLE TEST_DB.TEST_SCHEMA.T_ARCHIVE (
...
-- ============================================================================
-- 6. PIPE: PIPE_T_ARCHIVE
-- ============================================================================

CREATE OR REPLACE PIPE TEST_DB.TEST_SCHEMA.PIPE_T_ARCHIVE
    AUTO_INGEST = TRUE
    INTEGRATION = 'NOTIF_INT_THESIS'
AS
COPY INTO TEST_DB.TEST_SCHEMA.T_ARCHIVE
FROM (
    SELECT
        ...
        md5(to_varchar(array_construct(T.$1, T.$2, T.$3))) row_hash,
        md5(to_varchar(array_construct(T.$1))) key_hash,
        T.$1, T.$2, T.$3
    FROM @TEST_DB.TEST_SCHEMA.EXT_STAGE_THESIS/archive/
...
================================================================================
  ...
  External Stage: EXT_STAGE_THESIS (ROOT)
      |
      +-- /archive/ --> PIPE_T_ARCHIVE --> T_ARCHIVE
...
================================================================================
  ...
  4. Test each pipe:
       ALTER PIPE TEST_DB.TEST_SCHEMA.PIPE_T_ARCHIVE REFRESH;
